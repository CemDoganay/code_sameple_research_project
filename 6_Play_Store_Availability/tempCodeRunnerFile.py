...
with open(csv_androidx_location, 'r') as csv_androidx, \
        open(csv_google_play_existance_with_Androidx_location, 'w', newline='') as csv_google_play_existance:
        ...
        if android_x == 'AndroidX':
            if request.status_code == 200:
                ...
                csv_google_play_existance_writer.writerow(
                    [app_name, "exist", "androix"])
            else:
                print('Web site does not exist')
                csv_google_play_existance_writer.writerow(
                    [app_name, "dont exist", "androix"])
        else:
            if request.status_code == 200:
                print('Web site exists')
                csv_google_play_existance_writer.writerow(
                    [app_name, "exist", "NOT androix"])
            else:
                print('Web site does not exist')
                csv_google_play_existance_writer.writerow(
                    [app_name, "dont exist", "NOT androix"])
